...
public class PushNotifications {
     ...
     *
     * @param tenantId The tenant ID for the Bluemix application that the Push
     * Notifications service is bound to.
     * @param pushSecret The credential required for Push Notifications service
     * authorization.
     * @param bluemixRegion The Bluemix region where the Push Notifications
     * service is hosted. For example, US_SOUTH_REGION.
     */
     ...
     * service is hosted. For example, US_SOUTH_REGION.
     *
     * @throws IllegalArgumentException If either the push application ID or the
     * secret is not found in the environment variables.
     */
    public static void init(String bluemixRegion) {
        String tenantId = getApplicationIdFromVCAP();
        String pushSecret = getPushSecretFromVCAP();
    ...
    public static void initWithApiKey(String tenantId, String apiKeyId, String region) {
        apiKeyIdIs = apiKeyId;
        ...
        if (tenantId != null && apiKeyId != null) {
            createPushEndPointUrl(tenantId, region);
        } else {
            PushServerSDKException exception = new PushServerSDKException(PushConstants.PushServerSDKExceptions.PUSH_INIT_EXCEPTION);
            logger.log(Level.SEVERE, exception.toString(), exception);
            throw exception;
     ...
     * @param listener Optional PushNotificationsResponseListener to listen to
     ...
     *
     * @param obj The object which needs to be serialized as json string.
     *
        ...
        if (secret != null) {
            ...
            try {
                if (accessToken == null || (apiKeyExpireyTime - (System.currentTimeMillis() / 1000)) < 0) {
                    auth = getAuthToken();
                    JSONObject json = new JSONObject(EntityUtils.toString(auth.getEntity()));
